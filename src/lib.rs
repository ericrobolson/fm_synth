...
impl Plugin for SineSynth {
    fn get_info(&self) -> Info {
        Info {
            name: "FmSynth".to_string(),
            vendor: "EricOlson".to_string(),
            unique_id: 6667,
    ...
    fn get_editor(&mut self) -> Option<Box<dyn Editor>> {
        if self.opened_editor {
            return None;
        }

        self.opened_editor = true;

        let mut synth = SynthEditor::new();
        return Some(Box::new(synth));
    }
}
...
impl Editor for SynthEditor {
    ...
    fn open(&mut self, window_ptr: *mut std::ffi::c_void) -> bool {
